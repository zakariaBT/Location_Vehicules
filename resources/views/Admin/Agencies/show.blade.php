...
@section('content')
    ...
    <div class="card-6 bg-gray">
        <div class="card-heading ">
            <h2 class="title">Gestion Agences</h2>
        </div>
        <div class="card-body">
            <div class="container">
            @if (session('success'))
     ...
     <table id="table_id" class="display">
        <thead>
            <tr class="table100-head">
                ...
                <th class="column">Numero d'agence</th>
        ...
        <tbody>
            @if($Agencies->count())
                ...
                <tr>
                    <td class="column">{{$agency->updated_at}}</td>
                    <td class="column">{{$agency->name}}</td>
                    <td class="column">{{$agency->AgencyNumber}}</td>
                    <td class="column">{{$agency->address}}</td>
                    <td class="column columnF">
                        <form class="d-inline" action="{{route('agencies.edit',$agency)}}" method="GET" >
                            @csrf
                            <button type="submit" class="btn btn-style"><i class="fas fa-pencil-alt"></i></button>
                        </form>
                        <form class="d-inline delete_agency " action="{{route('agencies.destroy', $agency)}}" method="POST" >
                             @csrf
                             @method('delete')
                ...
                <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <h5 class="modal-title" id="exampleModalLabel">Voulez vous supprimez cette Agence?</h5>
